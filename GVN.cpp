...
//add new opt -options 
static cl::opt<bool> LVN("lvn", cl::init(false));
static cl::opt<bool> SVN("svn", cl::init(false));
...
uint32_t GVN::NumOfPredecessors(const BasicBlock* BB)
{
    uint32_t num=0;
...
bool GVN::is_ancestor(const BasicBlock *A, const BasicBlock * B)
{
  ...
  if (NumOfPredecessors(B)>1 || NumOfPredecessors(B)==0)
    return false;
...
void GVN::ConstructEBB(std::vector<const BasicBlock*>& EBB_, const BasicBlock* _Entry)
{
  for(const BasicBlock* BB:successors(_Entry))
  {
    if(NumOfPredecessors(BB)==1)
    { 
      EBB_.push_back(BB);
      ConstructEBB(EBB_,BB);
      continue;
    }
  }
  return;
...
void GVN::show_EBB(Function &F)
{
  if(SVN)
  {
    ///to output EBB information
    std::vector<const BasicBlock*> EBB_entries;

    for (Function::iterator FI = F.begin(), FE = F.end(); FI != FE;FI++ ) {
      const BasicBlock *BB = &(*FI);
      uint32_t num=NumOfPredecessors(BB);
      if(num==0 || num>1)
      {
        EBB_entries.push_back(BB);
      }
    }


    std::vector<std::vector<const BasicBlock*>> EBB_List;

    for(std::vector<const BasicBlock*>::iterator I=EBB_entries.begin(), E=EBB_entries.end();I!=E;I++)
    {
      ...
      EBB_List.push_back(EBB);
    }

    errs()<<'\n';
    errs()<<(F.getName())<<":"<<'\n';
    for(std::vector<std::vector<const BasicBlock*>>::iterator EBBL_I=EBB_List.begin();EBBL_I!=EBB_List.end();EBBL_I++)
    {
      std::vector<const BasicBlock*> EBB=(*EBBL_I);
      errs()<<"{";
      for(std::vector<const BasicBlock*>::iterator EI=EBB.begin(); EI!=EBB.end();EI++)
      {
        if(EI!=EBB.begin())
        {
          errs()<<", ";
        }
        const BasicBlock* BB=(*EI);
        errs()<<(BB->getName());
      }
      errs()<<"}"<<'\n';
